...
const tableCompatibleActions = Object.fromEntries(
	Object.entries(actionSchemas).filter(([_key, { output }]) => {
		return (
			output.def.type === "array" && output.def.element.def.type === "object"
		);
	}),
) as {
	...
	Object.entries(tableCompatibleActions).map(([action, { output }]) => [
		action,
		{
			input: z.object({
				columns: z.array(
					z.object({
						children: scope(REACT_NODE, {
							context: Object.fromEntries(
								Object.entries(output.def.element.def.shape).map(
									([field, value]) => [`$$.table<${action}>.N.${field}`, value],
								),
							),
							name: `table<${action}>`,
						}),
					...
					{data.map((_row, i) => {
						return (
							<tr key={`row-${i.toString()}`}>
								{columns.map((p, i) => {
									return <td key={`column-${i.toString()}`}>{p.children}</td>;
								})}
							</tr>
						);
					})}
